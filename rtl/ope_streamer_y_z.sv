...
`endif
  .DW ( DW ),
  .UW ( UW ) ) tcdm_cast ( .clk ( clk_i ) );

hwpe_stream_intf_stream #( .DATA_WIDTH ( DATAW ) ) out_stream ( .clk( clk_i ) );

hci_streamer_ctrl_custom_t  source_ctrl;
hci_package::hci_streamer_flags_t source_flags;

// Assign input control buses to the relative ID in the vector.
assign source_ctrl      = ctrl_i.y_stream_source_ctrl;
  ...
  hci_core_fifo #(
    ...
    .tcdm_initiator ( load_fifo_d )
  );


  assign tcdm_cast.r_data = load_fifo_q.r_data;

  // Left TCDM buses assignment.
  assign load_fifo_q.req      = tcdm_cast.req;
  assign tcdm_cast.gnt        = load_fifo_q.gnt;
  assign load_fifo_q.add      = tcdm_cast.add;
  assign load_fifo_q.wen      = tcdm_cast.wen;
  assign load_fifo_q.data     = tcdm_cast.data;
  assign load_fifo_q.be       = tcdm_cast.be;
  assign load_fifo_q.r_ready  = tcdm_cast.r_ready;
  assign load_fifo_q.user     = tcdm_cast.user;
  assign load_fifo_q.id       = tcdm_cast.id;
  assign tcdm_cast.r_valid    = load_fifo_q.r_valid;
  // do not assign tcdm_cast.r_data = load_fifo_q.r_data
  assign tcdm_cast.r_opc      = load_fifo_q.r_opc;
  assign tcdm_cast.r_user     = load_fifo_q.r_user;
  assign tcdm_cast.r_id       = load_fifo_q.r_id;
  assign load_fifo_q.ereq     = tcdm_cast.ereq;
  assign tcdm_cast.egnt       = load_fifo_q.egnt;
  assign tcdm_cast.r_evalid   = load_fifo_q.r_evalid;
  assign load_fifo_q.r_eready = tcdm_cast.r_eready;
  assign load_fifo_q.ecc      = tcdm_cast.ecc;
  assign tcdm_cast.r_ecc      = load_fifo_q.r_ecc;
...
assign flags_o.y_stream_source_flags = source_flags;
