...
FPU=(0 1) # Mux = 1, Sys = 0; You can choose among (0),(1),(0 1)
HEIGHTS=(4 8 16)
ACCS=(16 32)
    ...
    for mux in "${READOUT[@]}"; do
      ...
      for fp in "${fp_combos[@]}"; do
        for size in "${sizes[@]}"; do
          ...
          SUMMARY_LINES+=(
            "$(printf "%2s x %2s   %-10s %3d %3d %3d   %2d     %d     %d   %-8s  %7s  %6s   %7s" \
            "$HEIGHT" "$HEIGHT" "$fp" "$size" "$size" "$size" "$FIFO" "$mux" "$fpu" "$status" "${cycles:--}" "${memreq:--}" "$eff")"
          )
        done
      done
    done
...
SUMMARY_LINES=(" Engine    Target     M   N   K   FIFO   MUX   FPU   Status       Cycles   MemReq   Util ")
SUMMARY_LINES+=("──────────────────────────────────────────────────────────────────────────────────")
# ─── Simulations ────────────────────────────────────────────────────────────────
