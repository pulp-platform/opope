...
#define MAX_ULP ((COMP_FMT == FP8  && MEM_FMT == FP16) ?  FP8FP16_MAX_ULP : \
                 ...
                 0u)
#define ABS_TOL_BITS ((COMP_FMT == FP8  && MEM_FMT == FP16) ?  FP8FP16_ABS_TOL_BITS : \
...
int opope32_compare_int(uint32_t *actual_z, uint32_t *golden_z, int len)
{
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (compare_fp32(actual_z[i], golden_z[i])) {
            errors++;
#ifdef DEBUG
        if(errors == 1) tfp_printf("FP32 error @ %d: act=0x%08x ref=0x%08x\n", i, actual_z[i], golden_z[i]);
